...
/**
  * The entry point to our domain, works with repositories and validations to implement behaviour
...
class PetService[F[_]](
  ...
  def udpate(pet: Pet)(implicit M: Monad[F]): EitherT[F, PetNotFoundError.type, Pet] = for {
  ...
  // In some circumstances we may case if we actually delete the pet; here we are idempotent and don't care
